...
export const computeWeightedAverageProbs = (allProbs: number[][], labelWeights: number[]): number[] => {
...
export const calculateAllDynamics = (poseData: NormalizedLandmarkList[]): { movement: number; rhythm: number } => {
  ...
  for (let i = 1; i < poseData.length; i++) {
    ...
    for (const jointId of keyJoints) {
      const p1 = poseData[i-1][jointId]; const p2 = poseData[i][jointId];
      if (p1 && p2) frameDistance += Math.sqrt(Math.pow(p2.x - p1.x, 2) + Math.pow(p2.y - p1.y, 2));
    }
  ...
  for (let i = 1; i < velocities.length; i++) totalAcceleration += Math.abs(velocities[i] - velocities[i - 1]);
...
export const preprocessData = (landmarks: NormalizedLandmarkList, scaler: ScalerInfoModel): number[] => {
  const rh = landmarks[KEY_JOINTS_MEDIAPIPE.RIGHT_WRIST], lh = landmarks[KEY_JOINTS_MEDIAPIPE.LEFT_WRIST], re = landmarks[KEY_JOINTS_MEDIAPIPE.RIGHT_ELBOW], le = landmarks[KEY_JOINTS_MEDIAPIPE.LEFT_ELBOW];
...
export const smoothLandmarks = (poseData: NormalizedLandmarkList[], windowSize = 5): NormalizedLandmarkList[] => {
  if (poseData.length < windowSize) return poseData;
  const indices = [KEY_JOINTS_MEDIAPIPE.RIGHT_WRIST, KEY_JOINTS_MEDIAPIPE.LEFT_WRIST, KEY_JOINTS_MEDIAPIPE.RIGHT_ELBOW, KEY_JOINTS_MEDIAPIPE.LEFT_ELBOW];
  return poseData.map((frame, i) => {
    ...
    for (const idx of indices) {
      ...
      for (const w of window) {
        mean.x += w[idx]?.x || 0; mean.y += w[idx]?.y || 0; mean.z += w[idx]?.z || 0;
      }
      mean.x /= window.length; mean.y /= window.length; mean.z /= window.length;
      newFrame[idx] = mean;
    }
    return newFrame;
  });
